...
export const TopCountries = forwardRef<HTMLDivElement, TopCountriesProps>(
  ({ items = [], isLoading }, ref) => {
    const maxScore = Math.max(1, ...items.map(i => i.score));

    return (
      <Card ref={ref} className="border-border bg-card h-full">
...
TopCountries.displayName = 'TopCountries';
