...
const assetTabs: { type: AssetType; label: string }[] = [
  ...
  { type: 'ip_range', label: 'IP Ranges' },
  { type: 'asn', label: 'ASNs' },
  { type: 'brand', label: 'Brands / Keywords' },
  { type: 'email_domain', label: 'Email Domains' },
  ...
  const handleSave = () => {
    ...
    if (editingAsset) {
      setAssets(prev => prev.map(a => a.id === editingAsset.id ? {
        ...a, value: formValue, label: formLabel, criticality: formCriticality,
        tags: formTags.split(',').map(t => t.trim()).filter(Boolean), updated_at: now
      } : a));
    } else {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Define the entities your organization wants to monitor. {assets.length} total assets.
          </p>
        </div>
        <Button onClick={openAdd} className="glow-cyan">
        ...
        <TabsList className="bg-secondary/50 border border-border">
          {assetTabs.map(tab => (
            <TabsTrigger key={tab.type} value={tab.type} className="text-xs data-[state=active]:text-primary">
              {tab.label}
              {totalByType(tab.type) > 0 && (
                <Badge variant="secondary" className="ml-1.5 h-5 px-1.5 text-xs">{totalByType(tab.type)}</Badge>
              )}
            </TabsTrigger>
          ))}
        </TabsList>
          ...
          {assetTabs.map(tab => (
            <TabsContent key={tab.type} value={tab.type}>
              {filtered.length === 0 ? (
                <EmptyState
                  icon="database"
                  title={`No ${tab.label} Added`}
                  description={`Add your organization's ${tab.label.toLowerCase()} to start monitoring for relevant threats.`}
        ...
        <DialogContent className="bg-card border-border">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input value={formValue} onChange={e => setFormValue(e.target.value)} placeholder="e.g. example.com" className="bg-secondary/30" />
            </div>
            <div>
              <label className="mb-1.5 block text-sm font-medium">Label (optional)</label>
              <Input value={formLabel} onChange={e => setFormLabel(e.target.value)} placeholder="Friendly name" className="bg-secondary/30" />
            </div>
            <div>
              <label className="mb-1.5 block text-sm font-medium">Criticality</label>
              <Select value={formCriticality} onValueChange={(v) => setFormCriticality(v as CriticalityTier)}>
                <SelectTrigger className="bg-secondary/30"><SelectValue /></SelectTrigger>
                <SelectContent>
                  {Object.entries(criticalityMap).map(([k, v]) => (
            ...
            <div>
              ...
              <Input value={formTags} onChange={e => setFormTags(e.target.value)} placeholder="web, production, critical-infra" className="bg-secondary/30" />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDialogOpen(false)}>Cancel</Button>
            <Button onClick={handleSave} disabled={!formValue.trim()} className="glow-cyan">
